...
contract SimpleForward is ERC223Token, usingOraclize{
    
    /*****
     * BASICS
     * ***/
    
    // read data from oraclize (simple case with 1 participant in oracle)
    string public receivedData;
    bytes public receivedString;
    string public addr;
    uint public step;
    event newDataMeasure(string receivedData);
    ...
    function SimpleForward() public{
        ...
        index = 0;
        prevTestTime = now;
        // comment or not: update data reading from URL using oracalize
        update();
    }
    ...
    function testFitnessScore() public {
        ...
        require(msg.sender == administration);
        // comment or not: update the data reading from URL using oracalize
        update();
    ...
    function collectData(address participant) public {
        ...
        // Oracalize, the steps here has been updated in __callback function
    ...
    // callback function after receiving data using oraclize using URL
    function __callback(bytes32 myid, string result) {
        if (msg.sender != oraclize_cbAddress()) throw;
        receivedData = result;
        newDataMeasure(receivedData);
        receivedString = bytes(receivedData);
        
        // do something with the temperature measure..
        uint splitPosition = 0;
        uint i = 0;
        // extract participants' address and steps (just 1 participant now)
        // steps
        for (i=0; i<receivedString.length; i++) {
            if (receivedString[i] == ":") {
                splitPosition = i-1;
            }
            uint c = uint(receivedString[i]);
            if (c >= 48 && c <= 57) {
                step = step * 10 + (c - 48);
            }
        }
        // address
        for (i=4; i<splitPosition; i++) {
            bytes(addr).push(receivedString[i]);
        }
        participants[bytesToAddr(bytes(addr))].steps = step;
    }
    
    // change address(string or bytes) read from oracalize to address
    ...
    function update() payable {
        ...
        oraclize_query("URL", "https://biots.yellowtech.ch/?min=1&max=10000");
    }
